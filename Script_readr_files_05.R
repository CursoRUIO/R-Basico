...
# read.table function
# archivo formato txt
data_txt1 <- read.table(file = "data_read_tab.txt",
                        sep = "\t",
                        dec = ",",
                        header = TRUE)
str(data_txt1)
...
data_txt2 <- read.table(file = "data_read_tab.txt",
                        sep = "\t", 
                        dec = ",",
                        header = TRUE,
...
data_xlsx <- read_excel("data_read.xlsx",
                        sheet = "datos",
                        col_names = TRUE,
                        na=c(""))
str(data_xlsx)
...
data_sav <- read.spss("data_read.sav", 
                       use.value.labels = F, 
                      to.data.frame = TRUE)
str(data_sav)
...
library(repmis)
ls("package:repmis")
data_dbx<-source_DropboxData(file = "data_read_tab.txt",
                                key="b2pnuajshfrr4as", sep="\t",
                                header = TRUE)
str(data_dbx)
